...
namespace Study.OOP.Builder
{
    public class StudyBuilder : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // Sword newSwordD = new SwordBuilder().SetRandomGrade().SetName().SetRandomAttackValue().CreateSword();
            // Sword newSwordE = new SwordBuilder().SetRandomGrade().SetName().SetRandomAttackValue().SetRandomElementType().CreateSword();
            // Sword newSwordF = new SwordBuilder().SetRandomGrade().SetName().SetRandomAttackValue().SetRandomElementType().SetRandomRandomOption().CreateSword();
            // 
            // Debug.Log(newSwordA.ToString());
            // Debug.Log(newSwordB.ToString());
            // Debug.Log(newSwordC.ToString());
            // Debug.Log(newSwordD.ToString());
            // Debug.Log(newSwordE.ToString());
            // Debug.Log(newSwordF.ToString());
            
            // 대장장이 활용해서 소드만들기
            // BlackSmith blackSmith = new BlackSmith();
            // for (int i = 0; i < 1000; ++i)
            // {
            //     Grade randGrade = (Grade)Random.Range(0, (int)Grade.END);
